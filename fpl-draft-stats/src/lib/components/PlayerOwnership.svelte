...
<Card.Root>
	<Card.Header>
		<Card.Title class="text-fpl-purple text-2xl">👥 Player Ownership</Card.Title>
	...
	<Card.Content class="space-y-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-6 bg-muted rounded-lg">
			<div class="flex flex-col gap-2">
				<label for="position-filter" class="font-semibold text-fpl-purple text-sm">Position:</label>
				<select
					id="position-filter"
			...
			{#each filteredOwnership as item}
				<div class="p-6 bg-muted rounded-lg border-l-4 {item.owners.length > 1 ? 'bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-400' : 'border-border'}">
					<div class="flex justify-between items-start mb-4 pb-4 border-b-2 border-border">
						<div>
							<h3 class="text-fpl-purple font-semibold text-lg mb-1">{item.player?.web_name || 'Unknown'}</h3>
						...
						<div class="flex flex-col items-center">
							<span class="text-3xl font-bold text-fpl-purple">{item.owners.length}</span>
					...
					<div class="grid grid-cols-4 gap-2 mb-4">
						<div class="flex flex-col items-center p-3 bg-background rounded-md">
							<span class="text-lg font-bold text-fpl-purple">{item.player?.total_points || 0}</span>
							<span class="text-xs text-muted-foreground uppercase">Points</span>
						</div>
						<div class="flex flex-col items-center p-3 bg-background rounded-md">
							<span class="text-lg font-bold text-fpl-purple">{item.player?.form || '0.0'}</span>
							<span class="text-xs text-muted-foreground uppercase">Form</span>
						</div>
						<div class="flex flex-col items-center p-3 bg-background rounded-md">
							<span class="text-lg font-bold text-fpl-purple">{item.starting}</span>
							<span class="text-xs text-muted-foreground uppercase">Starting</span>
						</div>
						<div class="flex flex-col items-center p-3 bg-background rounded-md">
							<span class="text-lg font-bold text-fpl-purple">{item.bench}</span>
							<span class="text-xs text-muted-foreground uppercase">Bench</span>
						</div>
					</div>

					<div>
						<strong class="text-fpl-purple text-sm">Owned by:</strong>
